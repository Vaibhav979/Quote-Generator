...
public class QuoteGenController {
    ...
    public ResponseEntity<?> getCurrentUser(Principal principal) {
        if(principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Not logged in"); // Unauthorized response if principal is null
        }
        String email = principal.getName();
        Option<User> user = userRepository.findByEmail(email).orElseThrow(() -> new RuntimeException("User not found"));
