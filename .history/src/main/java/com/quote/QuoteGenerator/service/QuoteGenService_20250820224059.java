...
public class QuoteGenService {
    ...
    public ResponseEntity<String> addQuote(Quotes quote, Principal principal) {
        ...
        message.setFrom("vaibhavsingh8931@gmail.com");
        message.setTo(user.getEmail());
        message.setSubject("Quote Added!");
        message.setText("Hi " + user.getName() + ", your quote has been added successfully!\n\n" + quote.getQuote());
        javaMailSender.send(message);

        return ResponseEntity.ok("Quote added and email sent.");
    }
    ...
    public ResponseEntity<String> deleteQuote(Long id, Principal principal) {
        ...
        if (!existingQuote.getUser().equals(user)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("You can only delete your own quotes");
        }
